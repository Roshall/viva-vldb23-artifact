...
while getopts q:s:c:p:f:v:d:e:x:w:l:o: flag
do
    case "${flag}" in
        q) query_name=${OPTARG};;
...
echo "Optimization target: ${opttarget}"
echo "==============================="
...
if [ ${platform} == "cpu" ]; then
    ...
    sed -i 's/gpu:.*/gpu: True/g' conf.yml
    platform=$(nvidia-smi --query-gpu=gpu_name --format=csv --id=0 | awk -F, 'NR==2{print}' | cut -d'-' -f1 | sed 's/Tesla //')
fi
...
logging_suffix="${input_video},${selectivity_fraction},${canary_input},${proxy_thresh},${f1_thresh},${costminmax},${hints_plan},${opttarget},${platform}"
